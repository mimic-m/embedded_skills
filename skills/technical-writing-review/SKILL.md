...
| 組み込み用語の不統一 | 「割り込み」「中断」「interrupt」、「レジスタ」「リポジトリ（誤変換）」等の混在 | HIGH |
| 初出未定義の略語 | アルファベット大文字略語（HAL, ISR, API, GPIO, UART, DMA等）が初出時に日本語展開されていない | HIGH |
...
| アンチパターン | 判定方法 | 深刻度 |
|---|---|---|
| トピックセンテンスの欠如 | 段落の最初の文が固有名詞・数値・コード・コマンドで始まっている（詳細優先の兆候） | MEDIUM |
| 1段落複数トピック | 段落内に「一方」「しかし」「ただし」「なお」「また」などの転換・追加を示す接続詞が2つ以上含まれる、または転換接続詞が1つ以上含まれかつ前後で主語が異なる | MEDIUM |
| 長すぎる段落 | 1段落が6文以上 | LOW |
...
| アンチパターン | 判定方法 | 深刻度 |
|---|---|---|
| スコープ未定義 | 冒頭（最初の段落または見出し直下）に「本文書は〜について説明します」「このガイドでは〜を解説します」のような目的文がない | HIGH |
...
| 〜を行う/実施する | 「を行う」「を実施する」（例: 「初期化を行う」→「初期化する」、「テストを実施する」→「テストする」） | LOW |
...
改善案が複数ある場合は最も自然な1つを選ぶ。「要確認」のみの指摘は出力しない。カテゴリ4「初出未定義の略語」の改善案は文章の書き換えではなく定義の挿入を示す（例: 「HAL」→「Hardware Abstraction Layer（HAL）」のように略語の直前に日本語名と英語正式名を補う）。
...
以下のカテゴリは問題が検出されませんでした（問題なしのカテゴリをすべて列挙する）。
...
ファイルパスが指定された場合は、リポジトリルート（`.git/` が存在するディレクトリ）からの相対パスとして `docs/reviews/YYYY-MM-DD-<ファイル名>-writing-review.md` に保存する。出力前にディレクトリを作成する。
